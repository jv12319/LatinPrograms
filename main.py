...
if int(which_quiz) == 1:
    iwords_missed = quiz_func(L.randomized_lat_adj_dic)
    while len(iwords_missed) != 0:
        ...
        if iretry_errors_answer.lower() == "y":
            iwords_missed = quiz_func(iwords_missed)
        else:
            print("Bye!")
    else:
        print("0 words missed. Great job!")
elif int(which_quiz) == 2:
    which_dec = input("Enter 1 for First Declension \n 2 for Second Declension \n 2.5 for Second Declension Neuter \n 3 for Third Declension \n Enter Num Here->")
    if float(which_dec) == 1:
        iiwords_missed = quiz_func(L.first_declension)
        while len(iiwords_missed) != 0:
            iiretry_errors_answer = input("Would you like to repeat the words you missed? Enter y or n " )
        if iiretry_errors_answer.lower() == "y":
            iiwords_missed = quiz_func(iiwords_missed)
        else:
            print("Bye!")
    else:
        print("0 words missed. Great job!")
