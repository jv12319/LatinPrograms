...
if int(which_quiz) == 1:
    ...
    if float(which_dec) == 1:
        #  words_missed1d = q.decl_func(L.first_declension)
        #  while len(words_missed1d) != 0:
        #     retry_errors_answer1d = input("Would you like to repeat the words you missed? Enter y or n " )
        #     if retry_errors_answer1d.lower() == "y":
        #         words_missed1d = q.decl_func(words_missed1d)
        #     else:
        #         print("Bye!")
        q.main_choice_retry_logic(q.decl_func,L.first_declension)
    elif float(which_dec) == 2:
        words_missed2d = q.decl_func(L.second_declension)
        while len(words_missed2d) != 0:
            retry_errors_answer2d = input("Would you like to repeat the words you missed? Enter y or n " )
            if retry_errors_answer2d.lower() == "y":
                words_missed2d = q.decl_func(words_missed2d)
            else:
                print("Bye!")
else:
